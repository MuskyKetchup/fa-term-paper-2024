...
1)  Разработка клиентского приложения с графическим интерфейсом:

    a)  Возможность ввода логина и пароля.

    b)  Отображение списка полученных и отправленных сообщений.

    c)  Ввод имени адресата и текста сообщения.

    d)  Удобный интерфейс для отправки сообщений и получения новых
        сообщений в режиме реального времени.

2)  Реализация серверной части:

    a)  Прием соединений от нескольких клиентов.

    b)  Обработка аутентификации через базу данных.

    c)  Расшифровка и пересылка сообщений между клиентами.

    d)  Сохранение сообщений в базе данных.

3)  Обеспечение безопасности данных:

    a)  Реализация собственного алгоритма легковесного транспортного
        шифрования (вместо TLS/SSL).

    b)  Шифрование данных перед отправкой и расшифровка на стороне
        получателя.

4)  Интеграция с базой данных MySQL:

    a)  Хранение информации о пользователях (имя, логин, пароль).

    b)  Сохранение истории сообщений с указанием отправителя,
        получателя, времени и текста сообщения.

    c)  Проверка данных при аутентификации пользователей.

5)  Архитектурное разделение:

    a)  Использование клиент-серверной модели на основе TCP-соединений.

    b)  Реализация многопоточности на стороне сервера для одновременной
        обработки нескольких клиентов.

    c)  Отказ от реализации общей «чат-комнаты» в пользу индивидуальной
...
1)  Пользователи и их аутентификация.

    a)  Каждому пользователю присваивается уникальный логин и пароль.
    ...
    b)  При входе в систему клиент передает учетные данные серверу, где
...
2)  Обмен зашифрованными сообщениями.

    a)  На стороне клиента перед отправкой текст шифруется с помощью
    ...
    b)  При получении сообщения клиент расшифровывает его, используя
...
3)  Хранение и управление сообщениями.

    a)  В базе данных для каждого сообщения сохраняются атрибуты:
        отправитель, получатель, дата/время отправки, текст (в
        зашифрованном виде), а также статус (прочитано/не прочитано).

    b)  Клиентское приложение позволяет пользователю просматривать как
        входящие, так и исходящие сообщения, сортировать их по времени
        или по собеседнику, а также архивировать долгую переписку при
        необходимости.

4)  Механизм оповещений и актуальность данных.

    a)  Реализована многопоточность на сервере, что позволяет
    ...
    b)  При поступлении нового сообщения сервер уведомляет адресата, и
        пользовательское приложение отображает всплывающее оповещение
        либо добавляет новое сообщение в интерфейс чат-окна.

5)  Интеграция с базой данных MySQL.

    a)  В таблице пользователей (например, users) хранятся данные об
        имени, логине и пароле. Дополнительно можно хранить
        метаинформацию: e-mail, дату регистрации, статус
        (онлайн/офлайн).

    b)  В таблице сообщений (например, messages) регистрируется вся
...
6)  Применение клиент-серверной архитектуры.

    a)  Серверная часть взаимодействует с базой данных и осуществляет
        авторизацию, маршрутизацию сообщений и контроль за
        одновременными сессиями.

    b)  Клиент написан на Java с использованием Swing для визуального
    ...
    c)  Обмен данными между клиентом и сервером происходит по протоколу
...
1)   Повышение уровня безопасности и конфиденциальности. Реализация
...
2)   Гибкость и кастомизация. Создание собственного решения позволяет
...
3)   Удобство для конечных пользователей. Графический интерфейс на Swing
...
4)   Контроль над данными и интеграция с внутренними системами. За счет
...
1)   Аутентификация: Клиент UserA вводит свои логин/пароль и нажимает
...
2)   Отправка сообщения: Когда UserA отправляет сообщение UserB, клиент
...
3)   Получение сообщения: Клиент UserB получает зашифрованные данные от
...
## **4.2 Реализация транспортного шифрования**
...
1)   users -- таблица пользователей. Хранит учетные записи для входа в
    чат.

Поля:

2)   id -- первичный ключ (INT, AUTO_INCREMENT), уникальный идентификатор
    пользователя.

3)   username -- имя пользователя (VARCHAR, уникальное).

4)   password -- пароль (VARCHAR).
...
5)   messages -- таблица сообщений. Сохраняет историю личных сообщений
    между пользователями.

Поля:

6)   id -- первичный ключ (INT, AUTO_INCREMENT), уникальный идентификатор
    сообщения.

7)   sender_id -- ID отправителя сообщения (INT, внешний ключ к
    users.id).

8)   receiver_id -- ID получателя сообщения (INT, внешний ключ к
    users.id).

9)   content -- текст сообщения (TEXT).
...
## **4.4 Реализация клиентского и серверного приложения**
...
1)   Запуск сервера: При старте сервер загружает драйвер MySQL и
...
2)   Ожидание подключений: В главном цикле серверного приложения
...
3)   Аутентификация клиента: Каждый поток ClientHandler сначала ожидает
...
4)   Обмен сообщениями: После авторизации поток ClientHandler переходит в
    ...
    a)   Если получатель онлайн (есть соответствующий объект
    ...
    b)   Если получателя нет в сети, сервер отправляет отправителю
...
5)   Отключение клиента: Если клиент закрывает соединение (например,
...
1)   Окно входа (LoginFrame): при запуске клиента появляется окно с
    ...
    a)   Клиент создает сокет, подключаясь к указанному адресу и порту.
        Если соединиться не удалось (сервер недоступен, неверный
        адрес/порт), выдается сообщение об ошибке («Не удалось
        подключиться к серверу»).

    b)   После успешного установления соединения клиент формирует строку
    ...
    c)   Клиент ждет ответ от сервера. Сервер вернет либо \"LOGIN_OK\" (в
...
2)   Окно чата (ChatFrame): после успешного входа открывается главное
    окно чата. В его заголовке отображается название («Чат») и логин
    текущего пользователя. Окно состоит из следующих элементов:

    a)   Метка с вашим логином и поле для ввода имени получателя (подпись
    ...
    b)   Большая текстовая область, где отображается переписка (чат).
    ...
    c)   Поле ввода нового сообщения и кнопка «Отправить». Пользователь
...
1)  Берет содержимое поля «Получатель» (имя адресата) и поля ввода
...
2)  Формируется строка формата \"MSG\|recepient\|text\", где recepient
...
3)  Зашифрованная строка отправляется на сервер через открытое
    соединение (PrintWriter сокета).

4)  После успешной отправки сообщение сразу же отображается в локальной
...
1)   Когда приходит строка, поток ее считывает, расшифровывает с помощью
...
2)   Если входящее сообщение начинается с \"ERROR\|\", значит сервер
...
## **4.6 Структура проекта и исходный код**
...
## **4.7 Клиентская часть (Java, Swing GUI)**
...
1)   Кроссплатформенность и удобство. Выбранный язык Java и библиотека
    Swing позволили создать наглядное приложение, которое может быть
    запущено на разных операционных системах.

2)   Безопасность передачи данных. Разработанный алгоритм шифрования
    гарантирует конфиденциальность сообщений.

3)   Гибкая архитектура. Клиент-серверная модель с применением
    TCP-соединений дает возможность легко масштабировать систему,
    обслуживая одновременно несколько пользователей.

4)   Модульность и перспектива развития. Реализованная структура кода и
...
5)   Практическая значимость. Полученный результат может служить основой
